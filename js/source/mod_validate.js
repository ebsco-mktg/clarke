...
var VALIDATE = (function ( pub ) {
	...
	pub.checkSubmitState = function() {
		var submit = $('.form-behavior input[type="submit"], .form-behavior button[type="submit"]').not('.noDisable'),
	...
	pub.errorsVisible = function() {
	  ...
	  /* added functionality for job roles 
  */
    ...
    var corporationRoles = [ 
      ...
      "Librarian",
      "Human Resources/Learning/Training",
    ...
    var medicalRoles = [ 
      ...
      // populates #job_role based on #market select
      pub.populateSelect = function() {
       
        cat=$('#market').val();
        $('#job_role').html('job role');
        $("#job_role").addClass("disabled").attr("disabled", $("#market").val() == "");
	...
	pub.init = function ( submitMode, varCfg ) {
		...
		EIS.loadOnce( 'jQuery', function() {
			...
			var phrase_state = 'State / Province',
				phrase_postal = 'Zip Code/Postal',
				phrase_postalOptional = 'Postal Code (optional)',
				phrase_notRquired = 'field not required';
				
			if ( typeof localDictionary === 'object' ) {
				window.console && console.log ('localDictionary exists: ', localDictionary );
				
				phrase_state = localDictionary.formFields.state;
				phrase_postal = localDictionary.formFields.postal;
				phrase_postalOptional = localDictionary.formFields.postalOptional;
				phrase_notRquired = localDictionary.formFields.notRequired;
			...
			countrySelect.change(function() {
				if ( countriesWithStates.indexOf(countrySelect.val()) !== -1 ) {
					...
					stateSelect.children('option').first().text(phrase_state+'*');
					postalCode.addClass('required').prev('label').text(phrase_postal+'*');
					...
					stateSelect.children('option').first().text(phrase_notRquired);
					postalCode.removeClass('required error').prev('label').text(phrase_postalOptional);
...
var PROGRESS = (function( pub ) {
	...
	*/
	...
	pub.setHash = function (numMoves) {
		...
		window.console && console.log('currPanel.idx: ', currPanel.idx);
	...
	pub.setProgress = function() {
		...
		if ( typeof localDictionary === 'object' ) {
			...
			phrase_step = localDictionary.progressBar.step;
			phrase_of = localDictionary.progressBar.of;
			
		} else {
			window.console && console.log ('localDictionary does not exist: ', typeof localDictionary );
			...
			phrase_step = dictionary[lang].step;
			phrase_of = dictionary[lang].of;
		}
		...
		if ( currPanel.idx !== -1 ) {
			...
			$('.indicator').css('width', currWidth + '%').text( phrase_step + ' ' + new Number(currPanel.idx + 1) + ' ' + phrase_of + ' ' + numPanels + ' ');
		}
